function p2 = mergeintervals( p )
%MERGEINTERVALS Merges overlapping start/stop pairs
% Removes pairs if start is greater than stop
% Sorts start/stop pairs based on start value
%
% Example 1:
%
% >> p
...
% Remove errant values where the start time is greater than the end time
p(p(:,1) > p(:,2), :) = [];
