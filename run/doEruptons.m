...
I = find(cell2mat(AKeruptions(2:end,4)) ~= -1 & cell2mat(AKeruptions(2:end,3)) >= params.VEI(1) & cell2mat(AKeruptions(2:end,3)) < params.VEI(2)); %has network and VEI > X
I = I + 1; %FIX: adjust for header val
vnames = unique(AKeruptions(I,5));

disp([int2str(size(AKeruptions(I,5),1)),' Eruptions analyzed:'])
disp(strcat(datestr((datenum(AKeruptions(I,1)))),' --> ',AKeruptions(I,5),' VEI: ',int2str(cell2mat(AKeruptions(I,3)))))
