function header = waveform2sacheader(w)
   ...
   header=set_sacheader2(header,'DEPMIN',min(get(w,'data')));
   header=set_sacheader2(header,'DEPMAX',max(get(w,'data')));
   header=set_sacheader2(header,'DEPMEN',mean(get(w,'data')));
   ...
   %set required header variables
   header=set_sacheader2(header,'IFTYPE',true);
   header=set_sacheader2(header,'LEVEN',true);
   header=set_sacheader2(header,'LCALDA',true);
   header=set_sacheader2(header,'LOVROK',true);
   version = 6;
   header=set_sacheader2(header,'NVHDR',version);
   head = 0;
   header=set_sacheader2(header,'LPSPOL',head);
   ...
   for n=1: numel(fieldsToChange)
      header = set_sacheader2(header,fieldsToChange{n}, get(w,fieldsToChange{n}));
   end
end
