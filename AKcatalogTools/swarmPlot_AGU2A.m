function handles = swarmPlot_AGU2A(vname, vlat, vlon, DateTime, lat, lon, Depth, Magnitude, maxz, eruption_windows, baddata, beta_output, t1, t2)
disp(mfilename)
% SWARMPLOT Plots describing the "swarminess" of eq clusters (according to
% Randy White).
% THIS VERSION LOOKS LIKE:
% - A singple plot with beta windows, eruption windows, and network down
% time
%
...
% title({[vname ' -- ANSS Catalog']})


%% Beta Plots

sp6 = [];
sp6.axh = subplot(1,1,1);
sp6.hline = betaPlot5B(vname, beta_output, eruption_windows, baddata);
sp6.axh.XTick = [];
addMagnitudes2BetaPlot_1A
