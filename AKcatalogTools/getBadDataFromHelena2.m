function baddata = getBadDataFromHelena2( volcname, windows, varargin )
...
%% THIS VERSION OF HB's file has problems, don't trust it
...
%     ib = VOLCANO1.baddata > min(min(windows)) & VOLCANO1.baddata < max(max(windows)); % index of days when the network is down within time period of interest
    ib = VOLCANO1.networkoff > min(min(windows)) & VOLCANO1.networkoff < max(max(windows)); % index of days when the network is down within time period of interest

    baddata = VOLCANO1.networkoff(ib); % list of days when network is down within time period of interest
